...
# Usage: ./nm-try.sh [--timeout-seconds SECONDS]
#
# To enable bash completion, add this to your ~/.bashrc:
#   source /path/to/nmcli-try.bash
...
if [[ "${BASH_SOURCE[0]}" != "${0}" ]]; then
    _nm_try_completion() {
        ...
        opts="--timeout-seconds --help -h"

        # If previous word was --timeout-seconds, suggest some common values
        if [[ "${prev}" == "--timeout-seconds" ]]; then
            COMPREPLY=( $(compgen -W "30 60 90 120 180 300" -- "${cur}") )
            return 0
        fi
    ...
    complete -F _nm_try_completion nm-try.sh
    complete -F _nm_try_completion nm-try
    complete -F _nm_try_completion ./nm-try.sh
    
    # Also register for the full path if it's in a standard location
    if [[ -f /usr/local/bin/nm-try ]]; then
        complete -F _nm_try_completion /usr/local/bin/nm-try
    fi
